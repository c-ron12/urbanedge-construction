...
const ProjectDetail = () => {
  ...
  React.useEffect(() => {
    if (location.state?.from === 'external') {   // If user came from external page (like LatestProjects) then scroll to top, because they might be in middle of the page when they click on Read More Button of ProjectCard. But if they are clicking on project from sidebar of ProjectDetail page, we don't scroll to top because they are already on the project detail page.
      window.scrollTo(0, 0);
    }
    fetchproject();
    fetchProjects();
