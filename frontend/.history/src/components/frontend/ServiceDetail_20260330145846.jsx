...
const ServiceDetail = () => {
  ...
  React.useEffect(() => {
    if (location.state?.from === 'external') {
      ...
      setTimeout(() => {
        // using setTiimeout because when clicked to footer service title the url changes and component starts redering and DOm may not be fully ready yet. so below we are trying, "document.querySelector('.section-9')". This may result in null.So we use SetTimeout for 100ms.
        const sidebarSection = document.querySelector('.section-9');
        if (sidebarSection) {
          sidebarSection.scrollIntoView({ behavior: 'auto' }); // scrool behaviour: auto, instant jump to the section-9 section
        }
      }, 100); // wait for render
    }
