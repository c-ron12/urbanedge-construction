...
const ProjectDetail = () => {
  ...
  React.useEffect(() => {
    if (location.state?.from === 'external') {
      window.scrollTo(0, 0); // If user came from external page (like  Home page or LatestProjects page) then scroll to top, because they might be in middle of the page when they click on a project. But if they are clicking on project from sidebar of ProjectDetail page, we don't scroll to top because they are already on the project detail page.
    }
    fetchProject();
    fetchProjects();
