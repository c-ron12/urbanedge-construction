...
const ServiceDetail = () => {
  ...
  const [loading, setLoading] = React.useState(true); // for main service details
  const [error, setError] = React.useState(false);

  const [servicesLoading, setServicesLoading] = React.useState(true); // for sidebar card services list
  const [servicesError, setServicesError] = React.useState(false);

  const fetchServices = async () => {
    setServicesLoading(true); // start loading
    setServicesError(false); // reset error
    try {
  ...
  const fetchService = async () => {
    ...
    try {
      ...
      const result = await res.json();
      setService(result.data); // set data
    } catch (err) {
      console.log(err);
        ...
        <div className="container py-5">
          <div className="row">
              ...
              <div className="card shadow border-0 sidebar-card">
                <div className="card-body px-4 py-4">
                  ...
                  >
                    ...
                    {servicesLoading && (
                      <SkeletonLoader bars={4} height="25px" width="100%" />
                    )}

                    {/* Sidebar error */}
                    {!servicesLoading && servicesError && (
                      <p className="text-danger">Failed to load services</p>
                    )}
            ...
            <div className="col-md-9 ps-md-4 ps-0 pe-0">
              ...
              {/* Main content error */}
              {!loading && error && (
                <p className="text-center text-danger pt-5">
