...
const LatestProjects = () => {
  ...
  const fetchLatestProjects = async () => {
    try {
      ...
      setProjects(result.data);   // data is coming from backend api response latest Projects function of ProjectsController.
    } catch (error) {
      console.log(error);
    } finally {
    ...
    <>
      <section className="section-3 bg-light pt-5 pb-1 pb-md-4">
        <div className="container-fluid pt-lg-5 pt-3 pb-4">
          ...
          {!loading && projects.length > 0 && (   // only show projects when not loading and projects exist
          ...
          {/* Empty State */}  // only show empty state when not loading and no projects exist
