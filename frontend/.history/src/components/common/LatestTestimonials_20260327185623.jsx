...
const LatestTestimonials = () => {
  ...
  const fetchLatestTestimonials = async () => {
    try {
      ...
      setTestimonials(result.data); // data from backend TestimonialsController
    } catch (error) {
      console.log(error);
    } finally {
  ...
  return (
    <section className="section-5 pt-5 pb-md-4">
      <div className="container pt-lg-5 pt-3 pb-5">
        ...
        {/* Loading skeleton, only show while fetching */}
        {loading && (
        ...
        {/* Empty state, only show when not loading and no testimonials exist */}
        {!loading && testimonials.length === 0 && (
