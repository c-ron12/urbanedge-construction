...
const Show = () => {
  ...
  const fetchArticlesBlogs = async () => {
    const res = await fetch(`${apiUrl}/articles-blogs`, {
      method: 'GET',
      headers: {
  ...
  const deleteArticleBlogs = async (id) => {
    ...
    if (confirm('Are you sure you want to delete this article?')) {
      ...
      if (result.status === true) {
        ...
        const filteredArticlesBlogs = articlesBlogs.filter(
          (articleBlogs) => articleBlogs.id !== id
              ...
              <div className="card shadow border-0">
                <div className="card-body">
                  ...
                  <table className="table table-striped">
                    ...
                    <tbody>
                      {articles &&
                        articles.map((articlesBlogs) => (
                          <tr key={articlesBlogs.id}>
                            <td>{articlesBlogs.id}</td>
                            <td>{articlesBlogs.title}</td>
                            {/* <td>{article.slug}</td> */}
                            ...
                            <td>
                              <Link
                                to={`/admin/articles-blogs/edit/${articlesBlogs.id}`}
                                className="btn btn-sm btn-info me-2"
                              >
                                Edit
                              </Link>
                              <Link
                                onClick={() => deleteArticlesBlogs(articlesBlogs.id)}
                                href="#"
