...
class TestimonialController extends Controller
{
    ...
    public function update(Request $request, $id)
    {
        ...
        // Handle Image Update Logic Here (similar to store)
        if ($request->input('imageId') > 0) {
            $tempImage = TempImage::find($request->input('imageId'));
            ...
            if ($tempImage) {
                ...
                // Optional: Delete old image file from disk if exists
                if ($testimonial->image) {
                    ImageHelper::deleteImageFiles($testimonial->image, 'testimonials');
                }
